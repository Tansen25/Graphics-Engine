...
{
	this -> drawWithOutline = true;
	this -> outlineColor = Vect(0.0f, 0.0f, 1.0f);
}

void PyramidObject::drawWithOutlineOn()
{
	this -> drawWithOutline = true;
}

void PyramidObject::drawWithOutlineOff()
{
	this -> drawWithOutline = false;
}

void PyramidObject::setOutlineColor(const Vect& inColor)
{
	this -> outlineColor = inColor;
}
...
void PyramidObject::setOutlineRenderState()
{
	GlobalVariableManager* gvm = GlobalVariableManager::getInstance();
	//Draw blue outline
	...
	//Set the wireframe render state
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	glLineWidth(gvm -> getWireFrameLineWidth());
...
void PyramidObject::draw()
{
	...
	if (drawWithOutline == true && gvm -> getWireFrameMode() == false)
	{
		//Draw blue wireframe version of geometry
		setOutlineRenderState();
